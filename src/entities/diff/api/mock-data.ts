    ...
    hunks: [
      {
        ...
        lines: [
          { id: 'l1', type: 'context', oldLineNumber: 10, newLineNumber: 10, content: 'export async function getUser(id: string) {' },
          { id: 'l2', type: 'removed', oldLineNumber: 11, newLineNumber: null, content: '  const response = await client.get(`/users/${id}`)' },
          { id: 'l3', type: 'added', oldLineNumber: null, newLineNumber: 11, content: '  const response = await client.get(`/users/${id}`, { withMeta: true })' },
          { id: 'l4', type: 'added', oldLineNumber: null, newLineNumber: 12, content: '  if (!response.ok) {' },
          { id: 'l5', type: 'added', oldLineNumber: null, newLineNumber: 13, content: '    throw new UserNotFoundError(id)' },
    ...
    hunks: [
      {
        ...
        contextBefore: null,
        lines: [
          { id: 'l1', type: 'context', oldLineNumber: 1, newLineNumber: 1, content: "import { logger } from './logger'" },
          { id: 'l2', type: 'removed', oldLineNumber: 2, newLineNumber: null, content: 'export function get(path: string) {' },
          { id: 'l3', type: 'added', oldLineNumber: null, newLineNumber: 2, content: 'export function get(path: string, options?: RequestOptions) {' },
